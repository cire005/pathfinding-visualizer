...
export default class PathfindingVisualizer extends Component {
  ...
  resetGrid() {
    ...
    for (let row = 0; row < freshGrid.length; row++) {
      for (let col = 0; col < freshGrid[0].length; col++) {
        const node = freshGrid[row][col];
        const nodeEl = document.getElementById(`node-${row}-${col}`);
        if (!nodeEl) continue;

        let baseClass = 'node';
        if (node.isStart) baseClass += ' node-start';
        if (node.isFinish) baseClass += ' node-finish';
        nodeEl.className = baseClass;
      }
    }

    this.setState({ grid: freshGrid, mouseIsPressed: false });
  }
